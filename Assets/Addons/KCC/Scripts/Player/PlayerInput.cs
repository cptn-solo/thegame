namespace Example
{
	...
	public sealed class PlayerInput : NetworkBehaviour, IBeforeUpdate, IBeforeAllTicks, IBeforeTick
	{
		...
		{
			if (_moveTouch == null && touch.Start.Position.x < Screen.width * 0.5f) { _moveTouch = touch; }
			if (_lookTouch == null && touch.Start.Position.x > Screen.width * 0.5f) { _lookTouch = touch; }
		}
