...
namespace Assets.Scripts.Services.Network
{
    ...
    /// <summary>
    /// Companion component for <see cref="NetworkStartCustom"/>. Automatically added as needed for rendering in-game networking IMGUI.
    /// </summary>
    [RequireComponent(typeof(NetworkStartCustom))]
    [AddComponentMenu("Fusion/Network Start GUI")]
    [ScriptHelp(BackColor = EditorHeaderBackColor.Steel)]
    public class NetworkStartGUICustom : Fusion.Behaviour
    {
        ...
        NetworkStartCustom _networkDebugStart;
        string _clientCount;
        bool _isMultiplePeerMode;

        Dictionary<NetworkStartCustom.Stage, string> _nicifiedStageNames;

#if UNITY_EDITOR

        protected virtual void Reset()
        {
            _networkDebugStart = EnsureNetworkStartCustomExists();
        ...
        protected virtual void Awake()
        {
            ...
            _nicifiedStageNames = ConvertEnumToNicifiedNameLookup<NetworkStartCustom.Stage>("Fusion Status: ");
            _networkDebugStart = EnsureNetworkStartCustomExists();
            _clientCount = _networkDebugStart.AutoClients.ToString();
            ValidateClientCount();
        }
        ...
        protected NetworkStartCustom EnsureNetworkStartCustomExists()
        {
            ...
            if (TryGetBehaviour<NetworkStartCustom>(out var found))
            {
                _networkDebugStart = found;
                return found;
            }

            _networkDebugStart = AddBehaviour<NetworkStartCustom>();
            return _networkDebugStart;
        }
        ...
        private void Update()
        {
            ...
            var nds = EnsureNetworkStartCustomExists();
            if (nds.StartMode != NetworkStartCustom.StartModes.UserInterface)
            {
                return;
            }

            var currentstage = nds.CurrentStage;
            if (currentstage != NetworkStartCustom.Stage.Disconnected)
            {
                return;
            }
        ...
        protected virtual void OnGUI()
        {
            ...
            var currentstage = nds.CurrentStage;
            if (nds.AutoHideGUI && currentstage == NetworkStartCustom.Stage.AllConnected)
            {
                return;
            }
            ...
            GUILayout.BeginArea(new Rect(leftBoxMargin, margin, width, Screen.height));
            {
                GUILayout.BeginVertical(GUI.skin.window);
                {
                    GUILayout.BeginHorizontal(GUILayout.Height(height));
                    {
                        ...
                        if (nds.AutoHideGUI == false && nds.CurrentStage == NetworkStartCustom.Stage.AllConnected)
                        {
        ...
        private void StartHostWithClients(NetworkStartCustom nds)
        {
            int count;
            try
            {
        ...
        private void StartServerWithClients(NetworkStartCustom nds)
        {
            int count;
            try
            {
        ...
        private void StartMultipleClients(NetworkStartCustom nds)
        {
            int count;
            try
            {
        ...
        private void StartMultipleAutoClients(NetworkStartCustom nds)
        {
            int.TryParse(_clientCount, out int count);
            nds.StartMultipleAutoClients(count);
        }

        private void StartMultipleSharedClients(NetworkStartCustom nds)
        {
            int count;
            try
            {
