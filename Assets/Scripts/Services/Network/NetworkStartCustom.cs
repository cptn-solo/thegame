...
namespace Assets.Scripts.Services.Network
{
    ...
    [AddComponentMenu("Fusion/Prototyping/Network Start")]
    [ScriptHelp(BackColor = EditorHeaderBackColor.Steel)]
    public class NetworkStartCustom : Fusion.Behaviour
    {
        ...
        {
            if (TryGetComponent<NetworkStartGUICustom>(out var ndsg) == false)
            {
                ndsg = gameObject.AddComponent<NetworkStartGUICustom>();
            }
        }
        ...
        protected virtual void Start()
        {
            ...
            if (existingrunner && existingrunner != RunnerPrefab)
            {
                if (existingrunner.State != NetworkRunner.States.Shutdown)
                {
                    ...
                    var gui = GetComponent<NetworkStartGUICustom>();
                    if (gui)
                    {
                        Destroy(gui);
                    }
            ...
            {
                if (TryGetComponent<NetworkStartGUICustom>(out var _) == false)
                {
                    gameObject.AddComponent<NetworkStartGUICustom>();
                }
            }
        }
        ...
        protected IEnumerator StartWithClients(GameMode serverMode, SceneRef sceneRef, int clientCount)
        {
            ...
            {
                Debug.LogWarning($"{nameof(NetworkStartCustom)} can't be a child game object, un-parenting.");
